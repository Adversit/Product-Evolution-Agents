...
输出以下字段（**扁平 JSON**：质量维度直接放在顶层 `dimensions`，**不要**嵌套 `quality` 对象，**不要**输出 status / status_history / total / gate / round，这些由代码填）：
...
按以下顺序输出**顶层** `dimensions` 数组（**不要**包在 `quality` 对象里），每维一个对象 `{name, score, rationale}`，name 必须精确等于：
...
**每个维度对象必须同时含 `name`（字符串）、`score`（0-100 整数）、`rationale`（字符串）三个字段，缺一不可**；务必输出全部 10 个维度。`rationale` 必须有具体锚点（引用簇/证据的事实），不许空话。
...
不要输出 `total` / `gate` / `round`——这些由代码根据你的 `dimensions` 计算覆写，你只需如实给 10 维评分。
